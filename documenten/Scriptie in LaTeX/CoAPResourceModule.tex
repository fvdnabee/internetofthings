...
\item Entity API \cite{entityApiModule}: Een veelgebruikte module (wordt namelijk opgenomen in de core van Drupal 8). Breidt de functionaliteit uit die met entities kan verwezenlijkt worden. Deze module wordt besproken in \ref{entityAPI}.
...
\item Views: Maakt het gebruik van views mogelijk.
\item Views UI: Stelt een user interface ter beschikking om eigen views te maken en te beheren.
\end{itemize}
...
\subsubsection{coap\_resource\_resources}
...
\begin{itemize}
\item uri: Dit is de volledige URI van de resource, dus IPv6-adres en URI-path. Tenzij het hier een embedded device betreft, dan bestaat de URI enkel uit een IPv6-adres. Dit is tevens de primaire sleutel van deze kolom, elke waarde moet dus uniek zijn.
...
\item deleted: ?
\end{itemize}
\subsubsection{coap\_resource\_users}
Deze tabel bevat een entry voor elk koppel user/resource. Op die manier kan per user worden bijgehouden wat een user met een bepaalde resource wil doen. Ze bestaat uit de volgende kolommen:
\begin{itemize}
\item uid: een unieke identificatie voor een gebruiker. Dit ID is overgenomen uit Drupal, daar hebben alle gebruikers ook een uniek ID. Deze kolom is tevens deel van de primaire sleutel en is dus uniek.
\item uri: De URI van de resource waarin de gebruiker ge\{i\}nteresseerd is. Dit is dezelfde URI als in de vorige tabel (coap\_resource\_resources), deze kolom vormt dus een foreign key. Deze kolom maakt tevens uit van de primaire sleutel en dus moeten de waarden ervan uniek zijn.
\item nid: een unieke identificatie van de node waaraan de resource van deze user gekoppeld is in Drupal. Deze kolom is ook deel van de primaire sleutel en dus moeten de waarden ervan ook uniek zijn in deze tabel.
\item device: deze waarde geeft aan of het hier een resource of een volledig embedded device betreft (0 = resource, 1 = device).
...
\item version\_time: ?
\end{itemize}
\subsubsection{coap\_resource\_values}
...
\begin{itemize}
\item hid: dit is een uniek ID voor de entry automatisch gegenereerd bij toevoegen aan de databank. Dit is de primaire sleutel, dus deze waarden moeten uniek zijn.
\item uid: dit is het ID van de user waarvoor deze waarde opgeslagen is. Dit is hetzelfde ID als in de coap\_resource\_users-tabel en vormt dus een foreign key.
\item uri: dit is de URI van de resource waarvan deze waarde komt. Dit is dezelfde URI als in de coap\_resource\_resources-tabel en vormt dus een foreign key.
\item content\_format: deze kolom bevat voor elke entry het formaat van het bericht, aangeduidt met een leesbare naam (bijvoorbeeld plain text, JSON, ...). 
\item payload: dit is de payload van het bericht in leesbare vorm.
\item parsed\_value: dit is, indien mogelijk, een waarde die geparset is uit de payload, bijvoorbeeld een numerieke waarde. Indien er geen waarde kon geparset worden bevat deze kolom gewoon de payload.
\item max\_age: geeft aan wat de geldigheidsduur is voor deze waarde, deze is default nul.
\item timestamp: het tijdstip waarop de waarde in de databank werd opgeslagen, deze wordt automatisch gegenereerd. Samen met de max-age kan men dus bepalen of de waarde nog geldig is.
